...
const { useOrderForm } = AgnosticOrderForm;
...
const useAddItemsTask = (mutateAddItem: addItemMutate<GraphQLResult>) => (
...
const useUpdateItemsTask = (
  mutateUpdateQuantity: updateItemMutate<GraphQLResult>
) => (fakeUniqueIdMapRef: React.MutableRefObject<FakeUniqueIdMap>) => {
  const { setOrderForm } = useOrderForm();
  const { logSplunk } = useSplunk();
...
type addItemMutate<T extends GraphQLResult> = (input: {
  variables: AddItemMutationVariables;
}) => Promise<T | undefined>;

type updateItemMutate<T extends GraphQLResult> = (input: {
...
const OrderItemsProvider = ({
  ...
  handleAddItem,
  handleUpdateManualPrice,
}: PropsWithChildren<OrderItemsProviderProps>) => {
  const { orderForm, setOrderForm } = useOrderForm();

  const fakeUniqueIdMapRef = useFakeUniqueIdMap();

  const enqueueTask = useEnqueueTask();
  const addItemsTask = useAddItemsTask(handleAddItem)(fakeUniqueIdMapRef);
  const updateItemsTask = useUpdateItemsTask(handleUpdateQuantity)(
    fakeUniqueIdMapRef
  );
  const setManualPriceTask = handleUpdateManualPrice
    ? useSetManualPrice(handleUpdateManualPrice)()
    : undefined;
  ...
  const setManualPrice = setManualPriceTask
    ? useCallback(
        (price: number, itemIndex: number) => {
